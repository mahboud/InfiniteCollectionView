...
- (void)downloadImage:(NSString *)file
    completionHandler:(void (^)(UIImage *image))completionHandler {
  NSURL *downloadURL = [[[baseURL URL] URLByAppendingPathComponent:file]
  ...
  NSURLSessionDownloadTask *downloadTask = [session
      downloadTaskWithURL:downloadURL
        completionHandler:^(NSURL *location, NSURLResponse *response,
          ...
          //imageWithContentsOfFile:location.path];
...
- (UIImage *)getImageForPath:(NSIndexPath *)path
  ...
  NSString *key = [NSString
      stringWithFormat:@"%04ld-%04ld", (long)realSection, (long)realItem];
